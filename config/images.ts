const images = {
  logoOne: "/assets/images/logo1.png",
  logoTwo: "/assets/images/logo2.png",

  person: "/assets/images/person.png",

  banner1: "/assets/images/21_9.png",
  banner2: "/assets/images/16_9.png",
  banner3: "/assets/images/4_3.png",

  swiperOne: "/assets/images/swiper/swiper (1).png",
  swiperTwo: "/assets/images/swiper/swiper (2).png",
  swiperThree: "/assets/images/swiper/swiper (3).png",
  swiperFour: "/assets/images/swiper/swiper (4).png",
  swiperFive: "/assets/images/swiper/swiper (5).png",

  infoCardOne: "/assets/images/items/item (1).png",
  infoCardTwo: "/assets/images/items/item (2).png",
  infoCardThree: "/assets/images/items/item (3).png",
  infoCardFour: "/assets/images/items/item (4).png",

  cardOne: "/assets/images/cards/card (1).png",
  cardTwo: "/assets/images/cards/card (2).png",
  cardThree: "/assets/images/cards/card (3).png",
  cardFour: "/assets/images/cards/card (4).png",

  qrOne: "/assets/images/QR/qr (1).png",
  qrTwo: "/assets/images/QR/qr (2).png",
  qrThree: "/assets/images/QR/qr (3).png",
  qrFour: "/assets/images/QR/qr (4).png",

  personOne: "/assets/images/person/person (1).png",
  PersonTwo: "/assets/images/person/person (2).png",
  personThree: "/assets/images/person/person (3).png",

  reviewOne: "/assets/images/review/review (1).png",
  reviewTwo: "/assets/images/review/review (2).png",
  reviewThree: "/assets/images/review/review (3).png",

  footer: "/assets/images/footer.png",
  cartImage: "/assets/images/cartImage.png",

  messenger: "/assets/images/messenger.png",
  whatsapp: "/assets/images/whatsapp.png",

  productOne: "/assets/images/product/product (1).png",
  productTwo: "/assets/images/product/product (2).png",
  productThree: "/assets/images/product/product (3).png",
  productFour: "/assets/images/product/product (4).png",

  productInfoOne: "/assets/images/productinfo/Group (1).png",
  productInfoTwo: "/assets/images/productinfo/Group (2).png",
  productInfoThree: "/assets/images/productinfo/Group (3).png",
  productInfoFour: "/assets/images/productinfo/Group (4).png",
  productInfoFive: "/assets/images/productinfo/Group (5).png",
  productInfoSix: "/assets/images/productinfo/Group (6).png",

  cart: "/assets/images/cart.png",

  user: "/assets/images/user.webp",

  frame1: "/assets/images/login/frame (1).png",
  frame2: "/assets/images/login/frame (2).png",
  frame3: "/assets/images/login/frame (3).png",
  frame4: "/assets/images/login/frame (4).png",
  frame5: "/assets/images/login/frame (5).png",
  social1: "/assets/images/login/social (1).png",
  social2: "/assets/images/login/social (2).png",
  social3: "/assets/images/login/social (3).png",

  productBanner: "/assets/images/products/banner.png",
  productImg: "/assets/images/products/Card 5.png",
  serviceBG: "/assets/images/products/Texture 10.png",

  paymentOne: "/assets/images/payment/payment (1).png",
  paymentTwo: "/assets/images/payment/payment (2).png",
  paymentThree: "/assets/images/payment/payment (3).png",
  paymentFour: "/assets/images/payment/payment (4).png",

  emptyTracking: "/assets/images/tracking/empty-box 1.png",
  // images for testing
  test1: "/assets/test/test (1).jpg",
  test2: "/assets/test/test (2).jpg",
  test3: "/assets/test/test (3).jpg",
  test4: "/assets/test/test (4).jpg",
  test5: "/assets/test/test (5).jpg",
  test6: "/assets/test/test (6).jpg",
  test7: "/assets/test/test (7).jpg",
  test8: "/assets/test/test (8).jpg",
};

export default images;
